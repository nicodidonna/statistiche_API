...
// includiamo database.php e verbali_pagati.php per poterli usare
include_once '../config/db.php';
include_once '../models/verbali_pagati.php';
...
$verbali_pagati = new VerbaliPagati($db_id);
...
if( isset($_GET['tipoRead']) ){
    ...
    if ( isset($_GET['data_inizio']) and isset($_GET['data_fine']) ) {
        ...
        $stmt = $verbali_pagati->read($GLOBALS['tipoRead'],$param,$param2);
    
    } else {
        
        $stmt = $verbali_pagati->read($GLOBALS['tipoRead']);
...
if ($num > 0) {
    ...
    echo json_encode(array("message" => "La ricerca di Verbali o Preavvisi pagati non ha prodotto nessun risultato."));
}
